...
> **Active design deltas (read with this document).** Two accepted deltas amend sections below;
> where they disagree with this rev-4 text, the delta is newer and wins for the sections it names.
>
> | Delta | Amends | Phases |
> |---|---|---|
> | [`OPTIMIZATION-LEVELS.md`](OPTIMIZATION-LEVELS.md) | §2 (region/path/width semantics), §3 (cost model + frequency), §5 (auto-correct policy) — adds the `-O0..-O3/-Os/-Oz` presets and ten dials, and records the measurement defects M1–M7. **P14 (M1–M6) and P15 (the `-O` surface) are implemented** — `measurement=delay-v1`, dials in `core/src/opt.rs`, as-built maps in §1.2 / §3.2; M7 (P19) + dial 8 remain open | P14 ✓ P15 ✓ / P19 |
> | [`PERF-CACHE.md`](PERF-CACHE.md) | §6 (cache schema v1) and the *Partial rebuild* algorithm — adds the pre-compiled per-file `units` tier (relaxes **KD15**, proposes **KD19**), fixes `crc_set` closure, lists throughput bottlenecks B1–B9 | P16–P18 |
>
> Status of the code against this document is tracked in [`../AGENTS-todo.md`](../AGENTS-todo.md).
...
> **Delta:** transform *strength* is being moved behind an explicit level surface
> (`-O0..-O3`, `-Os`, `-Oz`) plus ten dials — see [`OPTIMIZATION-LEVELS.md`](OPTIMIZATION-LEVELS.md).
> The hard gates below are **orthogonal** to that surface and are never relaxed by a level.
> **As built (P14):** cut selection is now **cost-balanced** (prefix-sum bisection, not the
> index midpoint) and `split_assign` no longer reports a gain it did not cause — a
> split-only pass ends the loop. `InsertReg` still requires `--allow-latency` + complete
> `GateInfo`.
...
**Strength (P15):** `-O0..-O3 / -Os / -Oz` + ten `--opt-*` dials select how hard the loop
tries; the resolved set is echoed (`opt=…`) and reported in the JSON `opt` block. `-O1`/`-Oz`
are latency-neutral by construction, `-Os` is flop-minimal, `-O3` pipelines to budget. Per
KD20 a level never relaxes a gate above.
...
> **Delta:** a pre-compiled per-file `units` tier is added between the file digests and the
> `modules` tier, so an IR miss **links from cached lowered units** instead of reparsing every
> clean sibling — see [`PERF-CACHE.md`](PERF-CACHE.md) §3. That delta also fixes the committed
> `crc_set` (primary file only today, must be the closure `F(m)`) and switches blobs from
> `serde_json` to bincode + zstd. Schema below is the shipped v1.
...
| KD19 | *(Proposed, P17 — see [`PERF-CACHE.md`](PERF-CACHE.md) §3.1)* IR miss **links from per-file `units` blobs** whose `(crc, parser_version, pp_fingerprint, unit_ir_version)` all match; any mismatch reparses that file only. Supersedes KD15's full-set reparse without weakening its intent (never use a stale parse). | Incremental analyze proportional to *changed* files |
| KD20 | *(Implemented, P15 — see [`OPTIMIZATION-LEVELS.md`](OPTIMIZATION-LEVELS.md) §3)* Optimization **level presets only set dials**; safety gates (allowlist, refuse lists, `--allow-latency`, dry-run, emit containment) are orthogonal and never relaxed by a level. No `-Ofast`. | Tunable strength without a safety back door |
