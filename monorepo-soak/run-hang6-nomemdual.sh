...
LOG=$ROOT/monorepo-soak/hang6-nomemdual.log
echo "[hang6-nomemdual] start $(date) harness=$(ls -la $H | awk '{print $5,$6,$7,$8,$9}')"
# 400k cycles: hang-6 historically ~125k
"$H" +time_out=400000 +debug_disable +tohost_addr=0x80041730 "$ELF" 2>&1 | tee "$LOG"
echo "[hang6-nomemdual] done exit=${PIPESTATUS[0]}"
echo "=== summary ==="
