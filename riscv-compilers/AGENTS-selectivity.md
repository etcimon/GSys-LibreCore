...
> repository** (a monorepo, worktree, or any superproject that already tracks `riscv-compilers/` as
...
>
> Nothing here names a language, build system, intermediate representation, or particular compiler.
> Those are outputs of admission and inventory, recorded in **untracked** process files or in a
> checkout’s own surface.
...
1. **Host pollution.** Registering every compiler checkout as a *host* git submodule, or committing
   session notes into the host, couples the host history to foreign trees and to unfinished
   analysis.
2. **Scaffold churn.** Writing compiler-specific facts, toolchain versions, or pass logs into the
...
|---|---|---|---|
| **Scaffold (tracked)** | `AGENTS.md`, `AGENTS-*.md`, `README.md`, `LICENSE`, `agents/README.md`, `architecture/README.md` | **yes** | Generic contract, ladder, method. No compiler inventory. |
...
| **Local tools (untracked)** | `toolchains/`, caches, build debris | **no** | Prior compilers and extracts used to run green cells. |
...
- a particular compiler’s name, pin, pipeline, or dependency graph
...
enumerating compiler names. A sufficient first-level rule is: ignore every directory under
`riscv-compilers/` except the scaffold directories `agents/` and `architecture/`.
...
6. Never invent ignore lines that name a specific compiler; use the generic directory pattern.
