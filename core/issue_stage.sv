...
#(
  ...
  if (CVA6Cfg.OoOEn) begin : gen_full_ooo
    ...
    begin : gen_unresolved_cf_stall
      ...
      // Note (I4s): an unresolved_link_q (x1/x5 after CTRL_FLOW) was tried and
      // regressed soft-ladder hold (cookie red / early hang) even when scoped to
      // CF-only — unresolved_cf already serializes until resolve. Keep only
      // sp + csr + cf gates here; hang-6 multi-call relies on same-cycle LOAD
      // cancel in scoreboard (I4s).
