...
| **RTL landing (iter-012 / I4s–I4at)** | I4s LOAD cancel + `unresolved_sp_q`. I4t/v/ah: SMT JALR/BP only if executable. **I4ad/e:** leftover-RVI. **I4ag:** execute is `.text`+payload. **I4ai–l:** cancel-exempt fp setup/save/`ld s0,off(sp)`. **I4ak:** FLU ALU0/port0 tid. **I4am:** CASQ-only `waddr=rd\|1`. **I4ao:** any unaligned `we_gpr`→s0 — **nat moved**. **I4as:** no page0 `we_gpr`→ra — **nat moved** (`ra=12bc8`, `npc=129f0`). **I4at:** keep `jal`/`jalr`→ra (did not fire). **I4ap/q/ar** did not fire. **I4af reverted.** Live pin: cookie `51b1c001` / `npc=129f0` `jal offset_ptr@128e8`. Pin `bc7ed11d…`; do not cold-`mk` from drifted diag. |
