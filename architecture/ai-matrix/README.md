...
| **P1** | option **B**: `AiMatrixEn`, T0/T1 `ai.mma.s8` + `ai.requant` behind `COPRO_G6LC_AI`; `g6lc64_ai` package; directed tests | lint all targets + directed + numerical golden |
| **P2** | PMU events, RVFI probes, DFT threading, accumulator `tc_sram` island | synth smoke + FO4 note |
