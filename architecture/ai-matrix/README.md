...
| **I1** | **one** island cluster: PE array, `tc_sram` banks, sequencer, capability window. Freezes `T`, accumulator geometry, DRAM class and the NoC cut line **for both SKUs**. **Landed (partial):** AccTile*=256 / PeLanes=256 multi-bank C + I3-lite B oct-drain + multi-beat AR/AW + dual-bank C-read + PMU BW in CAP DRAM; full NoC open | P3 |
