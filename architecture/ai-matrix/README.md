...
| **I1** | **one** island cluster: PE array, `tc_sram` banks, sequencer, capability window. Freezes `T`, accumulator geometry, DRAM class and the NoC cut line **for both SKUs**. **I1-lite:** sequential GEMM + `g6lc_ai_tile_sram` A/B/C banks (MaxDim=8); PE array still open | P3 |
