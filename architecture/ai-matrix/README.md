...
| **I1** | **one** island cluster: PE array, `tc_sram` banks, sequencer, capability window. Freezes `T`, accumulator geometry, DRAM class and the NoC cut line **for both SKUs**. **Landed (partial):** AccTile*=256 / PeLanes=128 multi-bank C + I3-lite dual-write B + multi-beat AR/AW + dual-bank C-read + PMU BW in CAP DRAM; full Macs/NoC open | P3 |
