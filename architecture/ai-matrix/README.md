...
| **I1** | **one** island cluster: PE array, `tc_sram` banks, sequencer, capability window. Freezes `T`, accumulator geometry, DRAM class and the NoC cut line **for both SKUs**. **I1-lite:** banked A/B tiles + `g6lc_ai_pe_dot` (PeLanes=4) + C bank (MaxDim=8); SKU-scale AccTile* still open | P3 |
