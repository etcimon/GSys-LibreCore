...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1 AccTile*=256 / PeLanes=256; I3-lite multi-beat + B oct-drain + trail C-store + multi-out AR (depth 2) + PMU→CAP.** 256^3 **83.7k cy**. Suite 28/28. **ai-tensor:** host stack + lab HARD bridge green (`run-ai-tensor-rtl-hard`: mmio+gemm_s8 PASS on work-ver-ai). Next: board UIO/eventfd; full HARD suite CI; completion FIFO RTL; wider NoC. |
