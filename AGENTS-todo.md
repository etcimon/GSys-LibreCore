...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1 AccTile*=256 / PeLanes=256; I3-lite + multi-out AR + PMU→CAP + CPL FIFO** (`g6lc_ai_cpl_fifo`, multi-claim directed PASS). 256^3 **83.7k cy**. Suite 28/28. **ai-tensor** host + lab HARD **post-FIFO green** (`work-ver-ai` mmio 1144 cy + gemm_s8 1067 cy, 2026-08-10). SoC multi-claim HARD PASS; run-ai-matrix-hard-suite.sh CI suite **27/27 PASS** (post-FIFO, no 128/256; 2026-08-11). EventFdWait hostless + FIFO irq re-arm; CI 27/27. Peak HARD 128x128 (21.9k cy) + 256x256 (83.7k cy) PASS; event-fd-soak monorepo. DTS ariane-ai + board-uio-eventfd contract. **virt-ai-pcie** virtual PCIe AI card board (`class: virtual`) + soft UIO/eventfd userspace (`ai-tensor/tools/virt_ai_card`, `run-virt-ai-card.sh`) for hostless card/SSH/BAR4 path CI. **Frameworks gate:** `Device(virt-card)` + structured PyTorch unittest `ai-tensor/python/tests/test_torch_virt_ai_island.py` via `cva6-build tensor pytorch\|regress --board virt-ai-pcie --core g6lc64_ai`. **Virtual implementation structure:** `tensor virt-impl` / `tensor pytorch --impl hard|full --rtl-hard` chains soft → SV HARD (`work-ver-ai` mmio+gemm_s8) → optional `--from-timing` sv-timing FO4 preflight (`run-ai-tensor-virt-impl.sh`; `architecture/ai-matrix/frameworks-virt-pcie.md` §2.1a). Next: kernel UIO/eventfd driver on board; torch wheels in CI optional hard gate; wider NoC. |
