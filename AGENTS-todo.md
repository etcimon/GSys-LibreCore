...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Live green on `ai-matrix-p1`.** Full T0/T1 + PMU/RVFI + acc `tc_sram` + **aiperm gate** + queue T0 stubs + DFT `testmode_i`. **`ai-matrix-veri` 9/9 PASS**. Still open: MBIST bind, FO4 note, smt2 ownership, real U-mode aiperm soak (PMP). |
| **AI-P3** | T2 descriptor engine scaffold | P3 | **Scaffold only:** `corev_apu/include/g6lc_ai_island_cfg_pkg.sv` + `corev_apu/ai_island/README.md` (not flisted). Next: desc engine + AI-3 address-check. |
...
| **AI-3** | T2 descriptor engine address-checking | P3 | **Not started** (island cfg package scaffolded). Must land *with* the engine: an unchecked DMA master reachable from a user-mapped doorbell page is privilege escalation. Scope grew with `isa-encoding.md` §7.1: the check is **per queue** and carries the submitting context, and accumulator/staging state must be scrubbed or bank-partitioned between queues. |
