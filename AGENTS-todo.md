...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Partial+++ on `ai-matrix-p1`.** T0 + tile/acc + multi-cycle MMA + **`ai.requant` RHE + `act.relu`**. Seam-B packed scale in GPR (`isa-encoding.md` §3.5). Suites: `ai-config-smoke`, `ai-matrix-directed`, **`ai-matrix-veri`** (live Variane, optional rebuild). Tests: csr, dot4, mma golden, **requant RHE golden**. Still open: live green on `work-ver-ai`, PMU/RVFI, island/`tc_sram`. |
