...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1 AccTile*=256 / PeLanes=256; I3-lite multi-beat + B oct-drain + trail C-store + multi-out AR (depth 2) + PMU→CAP.** 256^3 **83.7k cy** (TB AR latency ~0). Suite 28/28. **ai-tensor:** SoftIsland + dual-oracle + cosim + multi-tile stream + WaitPolicy + TF high-level + C ABI header + `run-ai-tensor-rtl` soft/hard. Next: wider NoC (axi2apb/CLINT 128b); board UIO/PLIC; HARD RTL on lab. |
