...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1-lite cfg-bound.** `gemm_seq` MaxDim/PeLanes from `IslandCfg.AccTileM`/`MacsPerCycle`; cap window + RTL co-freeze at 8/4. Directed through `ai_gemm_s8_8x8_smoke` + `ai_cap_bringup_smoke`. Next: scale AccTile*/Macs toward SKU target; measure DRAM BW (I3). |
