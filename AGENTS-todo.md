...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1 AccTile*=256 / PeLanes=256; I3-lite multi-beat + B oct-drain + trail C-store + multi-out AR (depth 2) + PMU→CAP.** 256^3 **83.7k cy**. Suite 28/28. **ai-tensor host stack saturated (hostless):** SoftIsland + cosim + stream/fetch + WaitPolicy/IRQ + depth/history + HostRuntime queue + ProbeReport/`probe.v1` + NumPy/TF + C ABI + `cva6-build tensor`. Next: board UIO/eventfd; HARD RTL; wider NoC. |
