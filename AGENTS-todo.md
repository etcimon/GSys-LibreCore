...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Partial++ on `ai-matrix-p1`.** Coprocessor + CSRs + sideband + **tile/acc RF + multi-cycle `ai.mma.*`** (one (m,n) full-K/cycle; issue stalls on `busy_o`). T0 `dot4`/`setcfg` + T1 `mvta`/`mvacc`/`relacc`/`mma`. Suites `ai-config-smoke` / `ai-matrix-directed` (17/0 compile gate). Tests: `ai_csr_*`, `ai_dot4_*`, **`ai_mma_s8_golden`**. Still open: live Variane, island/`tc_sram`, requant golden, PMU/RVFI. |
