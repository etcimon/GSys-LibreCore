...
| **AI-P3** | T2 descriptor engine + SoC attach | P3 | **Spine + AXI + sideband + PLIC-8.** Island MMIO on GPIO `0x4000_0000`; enq sideband + dual-mode poll; dual-enq drain protocol; **PLIC source ID 8** (`ai_irq_plic_smoke`). Next: DMA desc fetch from enq ptr, GEMM (I1). |
