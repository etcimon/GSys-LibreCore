...
| **AI-P3** | T2 descriptor engine + SoC attach | P3 | **Spine + AXI attach.** Island MMIO on GPIO window `0x4000_0000` when `MatrixEn` (`g6lc_ai_island_apb` + axi2apb). Standalone `ai-island-veri` + SoC `ai_island_mmio_smoke`. Next: PLIC IRQ, DMA master, GEMM (I1). |
