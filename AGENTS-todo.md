...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Partial — coprocessor RTL landed on `ai-matrix-p1`.** `core/cvxif_g6lc_ai/` (instr pkg mask/match custom-2 `0x5B`, `g6lc_ai_exec` T0 + T1/T2 stubs, `g6lc_ai_coprocessor`) wired at `corev_apu/src/ariane.sv` `gen_COPRO_G6LC_AI`, on `Flist.cva6` / `Flist.cva6_gate`. Functional: `ai.setcfg`/`getcfg`/`relacc`/`dot4*`. Stubs: MMA/requant/queue/expsel. Still open for full P1 gate: directed tests (`verif/tests/custom/ai/`), CSR surface + AI-X, real tile/acc `tc_sram`, numerical golden. Decision: **B** for P1–P2, **D** for small/embedded SKU. |
...
| **AI-X** | **`mstatus.xs` read-only summary driven from `aistatus.ais`** | P1 | **Open; RE-SCOPED by AI-E3 and now ordered AFTER the AI CSRs.** It is *not* "delete the `xs = Off` line" — that line is correct until an extension with state exists. Required shape: (1) add `aistatus` with the `ais` field; (2) make `core/csr_regfile.sv:1791` compute `xs` as a read-only function of `ais` **gated on `AiMatrixEn`**, keeping the forced `Off` otherwise so all existing packages stay bit-identical; (3) same for `vsstatus_d.xs` `:1425` under `RVH`; (4) `sd` at `:2242` needs no change. Writes to `mstatus.xs` are ignored, never trapped. Contract: `isa-encoding.md` §5. |
...
| **AI-3** | T2 descriptor engine address-checking | P3 | **Not started.** Must land *with* the engine: an unchecked DMA master reachable from a user-mapped doorbell page is privilege escalation. Scope grew with `isa-encoding.md` §7.1: the check is **per queue** and carries the submitting context, and accumulator/staging state must be scrubbed or bank-partitioned between queues. |
...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **Not started; UNBLOCKED** (AI-0 and AI-S1 both closed). `corev_apu/ai_island/**` and `corev_apu/include/g6lc_ai_island_cfg_pkg.sv` are tier R. Ordering rule: **the bandwidth target is measured at I3 before I2 grows the cluster count.** Staging regression gate: every I2 change must leave the latency-SKU results bit-identical and its measured bandwidth unchanged — if not, the cluster boundary leaked and the SKUs have diverged. |
