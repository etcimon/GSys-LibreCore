...
| **AI-X** | **`mstatus.xs` hardwired `Off`** | P1 | **Open, prerequisite for AI state.** `core/csr_regfile.sv:1791` (and `vsstatus_d.xs` `:1425`) force `xs = Off`; `mstatus.sd` already ORs `xs == Dirty` (`:2242`). Lift the hardwire **gated on `AiMatrixEn`** so all existing packages stay bit-identical. Contract: `isa-encoding.md` §5. |
