...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Open and fully unblocked** (AI-0 closed). `COPRO_G6LC_AI` is now in `config_pkg::copro_type_t`; next is the `corev_apu/src/ariane.sv:159-188` genblock arm and the mask/match table per `isa-encoding.md` §10. Decision recorded: **B** for P1–P2, **D** for the small/embedded SKU; **A** (inside Ara) and **C** (unified arbiter) rejected. Evidence table in `architecture/ai-matrix/README.md` §2. |
