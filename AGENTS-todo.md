...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Partial+ on `ai-matrix-p1`.** Coprocessor + CSRs + **CSR↔copro sideband** (`ai_aicfg`/`ai_ais` out, `setcfg`/`dirty` in) through `csr_regfile`→`cva6`→`ariane`→`g6lc_ai_coprocessor`. Issue rejects when `ais==Off`. Suites: `ai-config-smoke`, `ai-matrix-directed` (optional in `defaults.ts`). Tests: `verif/tests/custom/ai/*`, `testlist_ai_matrix.yaml`. Still open: live Variane run of smokes, tile/`tc_sram` + real MMA, numerical golden, PMU/RVFI. |
