...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1-lite + tile SRAM.** `g6lc_ai_tile_sram` (`tc_sram` dual-port Latency=0) for A/B/C; `g6lc_ai_gemm_seq` load→MAC→store; engine `ST_GEMM`; AXI mux store>gemm>fetch. Directed: `ai_gemm_s8_*`. Next: PE array (multi-MAC/cycle) + freeze `T`/DRAM class in capability window. Ordering: measure BW at I3 before I2 cluster replication. |
