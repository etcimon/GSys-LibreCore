...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1-lite in progress.** `g6lc_ai_gemm_seq`: load A/B tiles → local flop buffers → MAC → store C (MaxDim=8); engine `ST_GEMM`; AXI mux store>gemm>fetch. Directed: `ai_gemm_s8_smoke`, `ai_gemm_s8_lda_smoke`, `ai_gemm_dim_err_smoke`. Next: replace flop tiles with `tc_sram` banks + PE array (true I1); freeze `T`/DRAM class. Ordering: measure BW at I3 before I2 cluster replication. |
