...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1 AccTile*=256 / PeLanes=256; I3-lite + multi-out AR + PMU→CAP + CPL FIFO** (`g6lc_ai_cpl_fifo`). 256^3 **83.7k cy**. Suite 28/28. **ai-tensor** host stack + lab HARD mmio/gemm. Next: rebuild HARD with CPL FIFO; board UIO; wider NoC. |
