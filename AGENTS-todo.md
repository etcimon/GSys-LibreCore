...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1-lite frozen in cap window.** Live: PeLanes=4, MaxDim=8, banked A/B + PE + C `tc_sram`; `AiIslandLatencyDefault` matches RTL; `AiIslandLatencySkuTarget` holds full-SKU placeholders. Directed: `ai_gemm_s8_*`, `ai_gemm_s8_4x4_smoke`, `ai_cap_bringup_smoke`. Next: scale MaxDim/PeLanes toward AccTile* 256; measure DRAM BW (I3). |
