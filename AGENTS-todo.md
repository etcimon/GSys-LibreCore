...
| **AI-S3** | Island RTL: cluster → memory → (latency SKU) → NoC/N clusters → PD | I1, I3, I2, I4 | **I1-lite PE.** Banked A/B `tc_sram` (t%PeLanes) + `g6lc_ai_pe_dot` (PeLanes=4 MACs/cycle) + C bank; engine `ST_GEMM`; suite `ai_gemm_s8_*`. Next: freeze SKU `T`/DRAM class in capability window; scale MaxDim/PeLanes toward AccTile*. Ordering: measure BW at I3 before I2 cluster replication. |
