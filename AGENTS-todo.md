...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Live green on `ai-matrix-p1`.** T0 + multi-cycle MMA + requant RHE + relu + PMU/RVFI + **acc on `tc_sram`** (`g6lc_ai_acc_bank`). **`ai-matrix-veri` 7/7 PASS** (csr, setcfg, illegal-off, dot4, mma, requant, pmu; Verilator 5.008). Still open: DFT/MBIST, island I1 (depends P3), smt2 ownership / queue doorbell. |
