...
| **AI-P3** | T2 descriptor engine spine | P3 | **Spine green.** `g6lc_ai_{desc_pkg,addr_check,cap_window,desc_engine,island_top}` + `verif/regress/ai-island-veri.sh` PASS (cap, good desc, AI-3 bad ptr/perm, bad ver, disabled). Next: AXI-lite + DMA master on testharness map; GEMM is I1. |
