...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Live green on `ai-matrix-p1`.** T0 + tile/acc + multi-cycle MMA + requant RHE + relu + **PMU group 4 + RVFI aicfg/aistatus**. **`ai-matrix-veri` 5/5 PASS** on `work-ver-ai` / `g6lc64_ai` (csr, dot4, mma, requant, pmu; Verilator 5.008). Still open: island/`tc_sram`, fuller suite. |
