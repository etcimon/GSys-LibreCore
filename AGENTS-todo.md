...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Partial — coprocessor + CSR/AI-X on `ai-matrix-p1`.** RTL: `core/cvxif_g6lc_ai/` (mask/match custom-2, T0 exec + T1/T2 stubs) @ `ariane.sv` `gen_COPRO_G6LC_AI`. CSRs in `csr_regfile.sv` (`0x801-0x804` URW — **0x800 is FTRAN**, address fix in `isa-encoding.md`; `0x5C0-0x5C2` SRW; `0x7C8` MRW). **AI-X done:** `mstatus.xs`/`vsstatus.xs` = `aistatus.ais` when `AiCfg.MatrixEn`. Directed: `verif/tests/custom/ai/ai_csr_aistatus_xs.S`, `ai_dot4_s8_smoke.S`. Still open: CSR↔coprocessor aicfg/ais sync ports, real tile/acc `tc_sram`, numerical golden, testlist + suite register. |
...
| **AI-X** | **`mstatus.xs` read-only summary driven from `aistatus.ais`** | P1 | **Done on `ai-matrix-p1`.** `aistatus` CSR + `mstatus_d.xs` / `vsstatus_d.xs` driven from `aistatus_d[7:6]` when `AiCfg.MatrixEn`; forced `Off` otherwise. `sd` ORs `xs==Dirty` (now uses `_d` so same-cycle). Writes to `mstatus.xs` ignored. Residual: coprocessor local `ais` not yet tied to CSR `aistatus` (issue-time Off gate still open after reset in the coprocessor). |
