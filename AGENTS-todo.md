...
| **AI-1** | Seam: CVXIF `COPRO_G6LC_AI` (option B) | P1 | **Live green on `ai-matrix-p1`.** T0 + tile/acc + multi-cycle MMA + requant RHE + relu. **`ai-matrix-veri` 4/4 PASS** on `work-ver-ai` / `g6lc64_ai` (csr, dot4, mma, requant; Verilator 5.008). SMT CSR bank AI sideband wired; TB hang probe uses `gen_single`. Still open: PMU/RVFI, island/`tc_sram`, fuller suite. |
